...
function analyzeResults(results) {
    ...
    for (const url of urls) {
        ...
        for (const v of violations) {
            const impact = v.impact || 'review';
            const count = v.nodes?.length || 0;
            const severity = SEVERITY_MAP[impact]?.label || 'Manual Review Required';

            if (severity === 'Must Fix') {
                mustFixCount += count;
            } else if (severity === 'Good to Fix') {
                goodToFixCount += count;
            } else {
                reviewCount += count;
            }
            ...
            (v.tags || []).forEach(tag => {
                if (tag.startsWith('wcag')) {
                    wcagCounts.set(tag, (wcagCounts.get(tag) || 0) + count);
                }
            });
        }
    }

    const totalWcagRules = 50; // Approximate number of WCAG 2.1 A/AA rules
    const coveredRules = new Set();
    wcagCounts.forEach((_, key) => {
        const rule = key.split(':')[0];
        if (rule) coveredRules.add(rule);
    });

    const automationCoverage = Math.round((coveredRules.size / totalWcagRules) * 100);

    return {
        pagesScanned: urls.length,
        pagesWithIssues: pagesWithViolations,
        mustFixCount,
        goodToFixCount,
        reviewCount,
        automationCoverage,
