...
| **Claude (Sonnet / Opus)** | Anthropic | Yes — used via the GitHub Copilot coding agent (which runs on Claude models) to implement features (including `discover` mode, `discover-top-pages.js` script, workflow integration, CI fixes, accessibility fix for `scrollable-region-focusable` on `.fallback-prompt-text` elements, the **Local Dedupe and Pattern Clustering** feature in `scripts/dedupe-utils.js`, and the daily-perf-improver workflow integration), write documentation, fix bugs, and fix the heading-order accessibility violation caused by h2 elements in dedupe summary cards (changed to h4 to avoid h2→h4 skip). | No | No |
