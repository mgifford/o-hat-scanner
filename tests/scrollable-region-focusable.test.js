...
describe('scrollable-region-focusable: fallback-prompt-text keyboard access', () => {
    ...
    test('fallback-prompt-text elements have role="region" and aria-label for screen reader context', () => {
        ...
        // The aria-label is generated by client-side JS; verify the JS source in the
        // static HTML includes both the role and aria-label attributes.
        expect(html).toContain('role="region"');
        expect(html).toContain('tabindex="0" role="region" aria-label="\'');
        // Verify the prompt label strings are present (used as aria-label values at runtime)
        expect(html).toContain('Executive summary prompt');
        expect(html).toContain('Dev team summary prompt');
        expect(html).toContain('Jira checklist prompt');    });
});
